...
create table if not exists questions (
  ...
  accept jsonb not null default '[]'::jsonb,  -- accepted answers for type='text' (empty => AI-graded)
  explanation text
);
...
-- Per-user study notes (Markdown) for each exam
...
-- security_invoker = false: the view runs with the owner's (postgres) rights so
-- it can read `choices` even though that table's RLS blocks direct user SELECT.
-- Safe because the view never selects `is_correct`.
...
-- Notes: only the owner (per-user, per-exam)
